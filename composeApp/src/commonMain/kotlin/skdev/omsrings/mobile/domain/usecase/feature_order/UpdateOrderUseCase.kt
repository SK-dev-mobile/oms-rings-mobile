...
class UpdateOrderUseCase(
    ...
    suspend operator fun invoke(order: Order): DataResult<Order, DataError> {
        return repository.updateOrder(order).notifyError(
            notificationManager
        ).ifSuccess {
            PushManager.sendPush(
                title = "Изменен заказ на ${it.data.date.toLocalDate().format(DateTimePattern.SIMPLE_DATE)}",
                content = "Первая часть номера заказа: ${OrderIdFormatter.getFirstPart(it.data.id)}"
            )
        }
    }
}